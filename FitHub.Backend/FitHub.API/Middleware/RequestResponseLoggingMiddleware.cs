...
namespace FitHub.API.Middleware;
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        finally
        {
            ...
            if (elapsed > SlowRequestThresholdMs)
            {
                ...
                // >>> Bugfix 27.10.2025: safe disk write (does not break response if folder is missing)
                try
                {
            ...
            // >>> Bugfix 27.10.2025: CRITICAL: restore original stream and copy body back so JSON error is visible
